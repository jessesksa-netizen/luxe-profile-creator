...
export function Splash({ onEnter, accent = "#a855f7", backgroundUrl, backgroundType = "image", backgroundBlur = 30, username }: { onEnter: () => void; accent?: string; backgroundUrl?: string | null; backgroundType?: string; backgroundBlur?: number; username?: string }) {
  ...
  const blur = `blur(${Math.max(backgroundBlur, 24)}px) brightness(0.5)`;
    ...
    >
      <div className="absolute inset-0 bg-black overflow-hidden">
        {backgroundUrl && backgroundType === "video" ? (
          <video src={backgroundUrl} autoPlay muted loop playsInline className="h-full w-full object-cover opacity-50" style={{ filter: blur }} />
        ) : backgroundUrl ? (
          <img src={backgroundUrl} alt="" className="h-full w-full object-cover opacity-50" style={{ filter: blur }} />
        ) : null}
